...
output "ssh_command_control_plane" {
  ...
  value = "ssh ubuntu@${cidrhost("192.168.8.0/24", 180)}"
}

output "kubeconfig_command" {
  description = "Command to retrieve kubeconfig from control plane"
  value = "ssh ubuntu@${cidrhost("192.168.8.0/24", 180)} 'sudo cat /etc/rancher/k3s/k3s.yaml'"
}
...
output "cluster_info" {
  ...
  value = {
    control_plane = {
      ...
      ips = [for i in range(var.control_plane_count) : cidrhost("192.168.8.0/24", 180 + i)]
    }
    workers = {
      count = var.worker_count
      cpu = var.worker_cpu
      memory = var.worker_memory
      ips = [for i in range(var.worker_count) : cidrhost("192.168.8.0/24", 185 + i)]
    }
